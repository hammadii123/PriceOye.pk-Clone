...
var filteredBrand = "all"; // Initially showing all products
var filteredPrice = 2000; // Max price filter

// Populate brand filter dropdown
for (var brands in watches) {
  brandFilter.innerHTML += `<option value='${brands}'>${brands}</option>`;
}
...
function displayProducts() {
  ...
  // Looped through the selected brand and price filter
  for (var brand in watches) {
    if (filteredBrand === "all" || filteredBrand === brand) {
      for (var product in watches[brand]) {
        const productDetails = watches[brand][product];
        const productPrice = parseInt(productDetails.price.replace("$", "").replace(",", ""));
        
        if (productPrice <= filteredPrice) {
          const productCard = document.createElement("div");
          productCard.classList.add("col-lg-3", "col-md-4", "col-sm-6", "mb-5");

          productCard.innerHTML = `
              <div class="card custom-card">
                  <img src="${productDetails.img}" class="card-img-top custom-card-img" alt="${productDetails.name}">
                  <div class="card-body">
                      <h5 class="card-title">${productDetails.name}</h5>
                      <p class="card-text">Full Name: ${productDetails.fullName}</p>
...
function selectBrand() {
  filteredBrand = brandFilter.value;
  displayProducts();
}

//price range selection
priceRange.addEventListener("input", function() {
  filteredPrice = priceRange.value;
  priceValue.textContent = filteredPrice;
  displayProducts();
});

// Initialized product display when page loads
